...
class LocalFileExplorer:
    ...
    def _get_raw_dataframe(self, file_path):
        with tables.open_file(file_path) as f:
            ins = self.fast_data_agg(f.root.bcm1fagghist[:][["data"]])
            # TODO: Change for columns for name
            df = pd.DataFrame(
                f.root.bcm1fagghist[:][
                    [col for col in f.root.bcm1fagghist.colnames if col != "data"]
                ]
            )
            df["data"] = ins
        return df
    ...
    def get_single_dataframe(self, fill_number, i):
        ...
        try:
            fdf = self._get_raw_dataframe(file_path)
            fill_number, start_date, dt = self.extract_data_from_name(file_path,
                                                                      fdf)
            fdf["dt"] = dt
        except Exception as e:
            print(e)
